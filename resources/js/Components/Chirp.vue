...
// Mendapatkan properti chirp dari parent
const props = defineProps(["chirp"]);

// Inisialisasi form untuk edit message dan gambar
const form = useForm({
    message: props.chirp.message || "",
    image: null, // Tambahkan properti untuk file gambar
});
...
<template>
    <div class="p-6 flex space-x-2">
        ...
        <div class="flex-1">
            ...
            <!-- Tampilkan Pesan dan Gambar -->
            <p v-else class="mt-4 text-lg text-gray-900">{{ chirp.message }}</p>
            <img
                :src="chirp.image ? `/storage/${chirp.image}` : ''"
                alt="Chirp Image"
                v-if="chirp.image"
            />
        </div>
